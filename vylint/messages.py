...
class _Message(object):
    """An individual vylint message.

    This should be accessed via the MESSAGES dict keyed by msg_id,
    e.g.

      from vylint.messages import MESSAGES
...
_messages = {
    "E001": {"msg": "Trailing Whitespace", "long_msg": None, "default": "E"},
    "E002": {
        "msg": "Tab indents",
        "long_msg": """
...
def print_messages():
    ...
    print("\nAvailable vylint checks")
    print("------------------------\n")
    for k, v in MESSAGES.items():
        print(
            " [%(default)s] %(id)s : %(string)s"
            % {"default": v.default, "id": v.msg_id, "string": v.msg_str}
        )
        if v.long_msg:
