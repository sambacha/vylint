...
def test_approve(c, w3, assert_tx_failed, get_logs):
    ...
    assert_tx_failed(
        lambda: c.approve(operator, OPERATOR_TOKEN_ID, transact={"from": someone})
    )

    # approve invalid token
    assert_tx_failed(
        lambda: c.approve(operator, INVALID_TOKEN_ID, transact={"from": someone})
    )
...
def test_setApprovalForAll(c, w3, assert_tx_failed, get_logs):
    ...
    assert_tx_failed(
        lambda: c.setApprovalForAll(someone, approved, transact={"from": someone})
    )
...
def test_mint(c, w3, assert_tx_failed, get_logs):
    ...
    # mint by non-minter
    assert_tx_failed(
        lambda: c.mint(someone, SOMEONE_TOKEN_IDS[0], transact={"from": someone})
    )
