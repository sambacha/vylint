    ...
    def __prepared_function(self, *args, **kwargs):
        if not kwargs:
            ...
            fn_abi = [
                ...
                if x["name"] == self._function.function_identifier
            ].pop()
    ...
    def Filter_remove(self, *values):
        def get_key(v):
            return (
                ...
                v.get("log_index"),
                v.get("transaction_index"),
            )

        values_to_remove = set([get_key(value) for value in values])

        queued_values = self.get_changes()
        self.values = [
            value for value in self.get_all() if get_key(value) not in values_to_remove
        ]
...
def get_contract_from_lll(w3):
    def lll_compiler(lll, *args, **kwargs):
        ...
        deploy_transaction = {"data": contract._encode_constructor_data(args, kwargs)}
...
def check_gas_on_chain(w3, tester, code, func=None, res=None):
    ...
    if gas_estimate < gas_actual:
        raise Exception(
            "Gas upper bound fail: bound %d actual %d" % (gas_estimate, gas_actual)
        )

    print(
        "Function name: {} - Gas estimate {}, Actual: {}".format(
