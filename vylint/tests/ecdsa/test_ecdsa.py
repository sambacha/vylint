...
def test_v0_signature(c):
    ...
    assert (
        recover_with_version(c, sig_without_version, "00", TEST_MESSAGE).lower()
        == signer
    )

    # with 27 as version value (27 = 1b)
    assert (
        recover_with_version(c, sig_without_version, "1b", TEST_MESSAGE).lower()
        == signer
    )
...
def test_v1_signature(c):
    ...
    # with v1 signature
    assert (
        recover_with_version(c, sig_without_version, "01", TEST_MESSAGE).lower()
        == signer
    )

    # with 28 signature (28 = 1c)
    assert (
        recover_with_version(c, sig_without_version, "1c", TEST_MESSAGE).lower()
        == signer
    )
