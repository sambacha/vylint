...
def test_create_membership_proof():
    ...
    assert proof == [
        Web3.sha3(NULL_HASH),
        Web3.sha3(Web3.sha3(leaves[0]) + Web3.sha3(leaves[1])),
    ]
...
def test_calcMerkleRoot(c, assert_tx_failed):
    ...
    assert (
        c.calcMerkleRoot(hashed_test_leaf, test_index, proof, call={"gas": 100000})
        == merkle.root
    )
...
def test_verifyMerkleProof(c):
    ...
    assert c.verifyMerkleProof(
        hashed_test_leaf, test_index, merkle.root, proof, call={"gas": 100000}
    )

    # Returns False if the index is wrong
    assert not c.verifyMerkleProof(
        hashed_test_leaf, test_index + 1, merkle.root, proof, call={"gas": 100000}
    )

    # Returns False if the root hash is wrong
    assert not c.verifyMerkleProof(
        hashed_test_leaf, test_index, WRONG_HASH, proof, call={"gas": 100000}
    )
