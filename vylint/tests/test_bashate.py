...
class TestVyper(base.TestCase):
    ...
    def test_main_no_files(self, m_vylintrun):
        ...
        m_vylintrun.return_value = m_run_obj
    ...
    def test_main_return_one_on_ioerror(self, m_vylintrun):
        ...
        result = vylint.main(["--verbose", "/path/to/fileA", "/path/to/fileB"])
        m_run_obj.check_files.assert_called_with(
            ["/path/to/fileA", "/path/to/fileB"], True, 79
        )
        expected_return = 1
        self.assertEqual(expected_return, result)
    ...
    def test_multi_ignore_with_slash(self):
        ...
        vylint.check_no_trailing_whitespace("if ", self.run)
        vylint.check_if_then("if ", self.run)
    ...
    def test_multi_ignore_mixed(self):
        ...
        vylint.check_no_trailing_whitespace("if ", self.run)
        vylint.check_if_then("if ", self.run)
        vylint.check_indents("  echo", self.run)
...
class TestVyperSamples(base.TestCase):
    ...
    def assert_error_found(self, error, lineno):
        ...
        for call in self.m_log_error.call_args_list:
            ...
            if args[0].startswith(error) and lineno == args[3]:
                error_found = True
        if not error_found:
            self.fail("Error %s expected at line %d not found!" % (error, lineno))

    def test_sample_E001(self):
        test_files = ["vylint/tests/samples/E001_bad.sh"]
        self.run.check_files(test_files, False)

        self.assert_error_found("E001", 4)

    def test_sample_E002(self):
        test_files = ["vylint/tests/samples/E002_bad.sh"]
        self.run.check_files(test_files, False)

        self.assert_error_found("E002", 3)

    def test_sample_E003(self):
        test_files = ["vylint/tests/samples/E003_good.sh"]
        self.run.check_files(test_files, False)
        self.assertEqual(0, self.run.error_count)

    def test_sample_E003_bad(self):
        test_files = ["vylint/tests/samples/E003_bad.sh"]
        self.run.check_files(test_files, False)
    ...
    def test_sample_E004_bad(self):
        test_files = ["vylint/tests/samples/E004_bad.sh"]
        self.run.check_files(test_files, False)
        self.assert_error_found("E004", 3)

    def test_sample_E006_bad(self):
        test_files = ["vylint/tests/samples/E006_bad.sh"]
        self.run.check_files(test_files, False)
    ...
    def test_sample_E006_bad_custom_max_line_length(self):
        test_files = ["vylint/tests/samples/E006_bad_custom_length.sh"]
    ...
    def test_sample_E011_good(self):
        test_files = ["vylint/tests/samples/E011_good.sh"]
        self.run.check_files(test_files, False)

        self.assertEqual(self.run.error_count, 0)

    def test_sample_E011(self):
        test_files = ["vylint/tests/samples/E011_bad.sh"]
        self.run.check_files(test_files, False)
    ...
    def test_sample_E012(self):
        test_files = ["vylint/tests/samples/E012_bad.sh"]
        self.run.check_files(test_files, False)
    ...
    def test_sample_E041(self):
        test_files = ["vylint/tests/samples/E041_bad.sh"]
        self.run.check_files(test_files, False)

        self.assert_error_found("E041", 4)

    def test_sample_E042(self):
        test_files = ["vylint/tests/samples/E042_bad.sh"]
    ...
    def test_sample_E043(self):
        test_files = ["vylint/tests/samples/E043_bad.sh"]
    ...
    def test_sample_for_loops(self):
        test_files = ["vylint/tests/samples/for_loops.sh"]
        self.run.check_files(test_files, False)
    ...
    def test_sample_comments(self):
        test_files = ["vylint/tests/samples/comments.sh"]
        self.run.check_files(test_files, False)

        self.assertEqual(0, self.run.error_count)

    def test_sample_E005(self):
        test_files = ["vylint/tests/samples/E005_bad"]
    ...
    def test_sample_E005_excluded(self):
        test_files = ["vylint/tests/samples/.E005_excluded"]
        self.run.check_files(test_files, False)

        self.assertEqual(0, self.run.error_count)

    def test_sample_E040(self):
        test_files = ["vylint/tests/samples/E040_syntax_error.sh"]
    ...
    def test_sample_E044(self):
        test_files = ["vylint/tests/samples/E044_bad.sh"]
        self.run.check_files(test_files, False)
    ...
    def test_sample_warning(self):
        ...
        test_files = [
            "vylint/tests/samples/E011_bad.sh",
            "vylint/tests/samples/E041_bad.sh",
        ]
    ...
    def test_ignore_heredoc(self):
        test_files = ["vylint/tests/samples/heredoc_ignore.sh"]
        self.run.check_files(test_files, False)
        self.assertEqual(0, self.run.error_count)
    ...
    def test_pre_zero_dot_one_sample_file(self):
        ...
        release of the vylint pypi package. There were no tests
        ...
        test_files = ["vylint/tests/samples/legacy_sample.sh"]
        self.run.check_files(test_files, False)
